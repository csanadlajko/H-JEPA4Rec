...
def positional_encoding_1d(embed_dim, lenght):
    ...
    pe = torch.zeros(lenght, embed_dim, device=device)

    ## numbers from 0 corresponding for position in the sequence
    position = torch.arange(0, lenght, device=device).unsqueeze(1)
    ...
    div_term = torch.exp(
        (torch.arange(0, embed_dim, 2, dtype=torch.float, device=device) * -(math.log(10000.0) / embed_dim))
    )
